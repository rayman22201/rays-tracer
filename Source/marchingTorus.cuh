//Implements a Torus Surface using Ray Marching
//Allows for both bisection and Ridders method
//Ray Imber a.k.a Rayman22201

#ifndef __MARCHINGTORUS__
#define __MARCHINGTORUS__
...
namespace wildDoughnut {
	...
	class MarchingTorus : public surfaceObject<MarchingTorus> {
	protected:
		//--------------------Surface Evaluation Functions-------------------------//
		__host__ __device__ inline float surfaceAtPoint( Vector point )
		{
			//Vector rotated = Vector( (point.values.x), (point.values.y * cosf(65) + point.values.z * -1.0f * sinf(65)), (point.values.y * sinf(65) + point.values.z * cosf(65)));
			Vector pt = (point - this->position);
			float minuend = ( pt.dot(pt) + (this->radius2 * this->radius2) - (this->radius1 * this->radius1) );
			float minuendSquared = (minuend * minuend);
			float subtrahend = ( (4.0f * (this->radius2 * this->radius2)) * ((pt.values.x * pt.values.x) + (pt.values.y * pt.values.y)) );
			return (minuendSquared - subtrahend);
		}
		...
		__host__ __device__ inline Vector gradientAtPoint( Vector point )
		{
			...
			float minuend = ( pt.dot(pt) + (this->radius2 * this->radius2) - (this->radius1 * this->radius1) );
			float minuendX = ((4.0f * pt.values.x) * minuend);
			float x = (minuendX - (8.0f * (this->radius2 * this->radius2) * pt.values.x) );
			
			float minuendY = ((4.0f * pt.values.y) * minuend);
			float y = (minuendY - (8.0f * (this->radius2 * this->radius2) * pt.values.y) );

			float z = ((4.0f * pt.values.z) * minuend);
...
#define TOLERANCE 0.00001
	...
	public:
		...
		//constructor
		__host__ __device__ MarchingTorus() { position = Vector(0,0,0); radius1 = 0; radius2 = 0; marchInterval = 0.0f; numIterations = 10; }
